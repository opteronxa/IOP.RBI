...
public class C_Session {    // === Login ===
    ...
    private final C_Files TAGS;

    public C_Session() throws Exception {
        this.TAGS = new C_Files();
        ...
        try (C_PublisRabbitMQ y_rabb = new C_PublisRabbitMQ()) {
            try (C_Redis y_reds = new C_Redis()) {
                ...
                for (String s: this.INFOMAP.keySet()) {
                    this.getEvent(y_rabb, y_lr.getUserID(), y_lr.getKey(), s);
                }
            } catch (Exception e2) {
    ...
    public void loadDevices(C_Redis y_reds, int _userId, byte[] _key) throws Exception {
        ...
        for (int i = 0; i < y_records.length(); i++) {
            ...
            JSONObject y_red = y_reds.getVId(d.getString("deviceNum"));
    ...
    // === Get Event ===
    public void getEvent(C_PublisRabbitMQ _rabb, int _userId, byte[] _key, String _deviceNum) throws Exception {
